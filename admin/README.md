...
- 날짜별 관리자 작업 로그와 최근 로그 조회
- `Ctrl+C` 및 `SIGTERM` 안전 종료
...
- `log 20`: 오늘 기록된 최근 로그 20줄을 표시합니다.

로그는 `logs/admin-YYYY-MM-DD.log`에 저장됩니다. 비밀번호와 JWT는 기록하지
않으며 `logs/`는 Git에서 제외됩니다.
...
complete 196
cancel 165
bulk-start 101 102 103
bulk-complete 101 102
bulk-cancel 104 105
```
...
다중 명령은 기존 백엔드의 `/bulk/status`, `/bulk/cancel` API를 사용합니다.
백엔드는 선택한 모든 주문을 먼저 검증하고, 조건에 맞지 않는 주문이 섞여
있으면 전체 요청을 실패 처리합니다.
